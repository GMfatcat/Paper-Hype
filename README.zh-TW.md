...
## 一個真實案例（Mode B）
一則語氣克制的中文 X 長文,介紹 Perplexity 的「Search as Code」架構,引用 `100% 準確率`、`token −85.1%`、`WANDR 2.5 倍`。Mode B 取文後實際開啟 Perplexity 原始研究文章核對,確認**每個數字都與原文吻合**——所以它**沒有**亂喊「造假」。最終評 **27/100 🟡**,並點出唯一真缺陷:貼文把**廠商自家的第一方評測(WANDR 當時根本還沒公開)當成客觀結果呈現**——可以讀,但要知道這是誰家的計分板。
